...
{
    public class EnableEnvironmentRunner
    {
        private readonly ILanguageProvider _languageProvider;
        private readonly IOctopusHelper _octopusHelper;

        public EnableEnvironmentRunner(IOctopusHelper octopusHelper, ILanguageProvider languageProvider)
        {
        ...
        public async Task<int> Run(DeploymentEnvironment environment, Machine machine = null)
        {
            ...
            try
            {
                if (machine == null)
                {
                    await _octopusHelper.Machines.EnableMachines(environment.Id);
                    System.Console.WriteLine(_languageProvider.GetString(LanguageSection.UiStrings, "EnvironmentEnabled"), environment.Name);
                }
                else
                {
                    var enabled = await _octopusHelper.Machines.EnableMachine(machine.Id, environment.Id);
                    if (!enabled)
                    {
                    ...
                    System.Console.WriteLine(_languageProvider.GetString(LanguageSection.UiStrings, "MachineEnabled"), machine.Name, environment.Name);
                }
            }
            catch (Exception e)
            {
